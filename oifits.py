...
class OI_VIS:
    ...
    def __init__(self, timeobs, int_time, visamp, visamperr, visphi, visphierr, flag, ucoord,
        ...
        if type(cflux) != type(None):
            self._cflux = np.array(cflux, dtype=double).reshape(-1)
        else:
            self._cflux = None
        if type(cfluxerr) != type(None):
        ...
        if not self.wavelength:
            ...
            for wavelength in self.wavelength.values():
                if len(wavelength.eff_wave) != len(wavelength.eff_band):
                    errors.append("eff_wave and eff_band are of different lengths for wavelength table '%s'" % wavelength.eff_wave.key)
